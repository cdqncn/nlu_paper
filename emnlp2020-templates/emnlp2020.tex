...
\begin{gather}
    ...
    Q_S,K_S,V_S=K_{I}^T,Q_I^T,H_S,W^V_S \\
...
Where $Q_I, K_I, V_I$ are intent query, key and value, $Q_S, K_S, V_S$ represent query, key and value of slot, $W_I^Q$,$W_I^K, W_I^V, W_S^V$ are weight matrix corresponding to $Q_I, K_I, V_I, V_S$; $Q_I$ and$ K_I$ are the transpose of $K_S$ and $Q_S$, and $A_I$ is the transpose of $A_S$.
